...
class Frontmodel extends CI_Model {
    ...
    public function modify() {
        ...
        $date_of_passport_valid         = $this->input->post('date_of_passport_valid');
        $visa_id                        = $this->input->post('visa_id');
        $date_of_issue_of_VISA          = $this->input->post('date_of_issue_of_VISA');
        $date_of_visa_valid_till        = $this->input->post('date_of_visa_valid_till');
        $cgpa_entry_field        = $this->input->post('cgpa_entry_field');
        $best_of_5        = $this->input->post('best_of_5');
        ...
        $data = array(
            ...
            'date_of_passport_valid'        => date("Y-m-d H:i:s",strtotime($date_of_passport_valid)),
            'visa_no'                       => $visa_id,
            'date_of_issue_of_visa'         => date("Y-m-d H:i:s",strtotime($date_of_issue_of_VISA)),
            'date_of_visa_valid_till'       => date("Y-m-d H:i:s",strtotime($date_of_visa_valid_till)),
            'cgpa'                    => $cgpa_entry_field,
            'best_of_five'                    => $best_of_5
        );
...
<table cellspacing="0" cellpadding="0" style="width: 700px; margin: 0 auto; border: 0;">
    ...
    public function delete_temp_payment($student_id,$ref_id){
        $this->db->delete('payment_details', array('ref_id' => $ref_id, 'sbi_payment_status' => 2)); 
        return TRUE;
    }
    ...
    public function update_payment_status($student_id,$ref_id,$transaction_id,$decrypt){
        ...
        $this->db->trans_begin();
            ...
            $this->db->where(array('ref_id' => $ref_id, 'sbi_payment_status' => 2))->update(tablename('payment_details'), $data);

            // add on 23082019
            $this->db->select('student_id, form_type');
            $this->db->from(tablename('payment_details'));
            $this->db->where('ref_id', $ref_id);
            $query = $this->db->get();
            $result = $query->row();

            //15012021
            if($result->form_type == 'application_fee'){
                $update = array(
                        'status'   => 1
                    );
                $this->db->where('id', $result->student_id)->update(tablename('student'), $update);
            }
            $this->db->where('id', $result->student_id)->update(tablename('student'), array('payment_option_show'=> '2019-12-31 23:59:59')); // update on 23082019
    ...
    //15012021
    public function getWhatPayment($ref_id){
        $this->db->select('student_id, form_type');
        $this->db->from(tablename('payment_details'));
        $this->db->where('ref_id', $ref_id);
        $query = $this->db->get();
    ...
    public function update_student_status($student_id){
            ...
            $this->db->where('id', $student_id)->update(tablename('student'), $update);
    }
    ...
    public function hos_payment_invoice_details($paymentId){
        ...
        $this->db->from(tablename('payment_details')." as t1");
        $this->db->join(tablename('student')." as t2","t2.id = t1.student_id", 'left');
        $this->db->join(tablename('course')." as t3","t3.id = t2.course_id",'left');
        $this->db->join(tablename('hostel')." as t4","t4.id = t2.hostel_name",'left');
    ...
    public function getPaymentStatus($student_id){
        $this->db->select('PD.*');
        $this->db->from(tablename('payment_details')." as PD");
        $this->db->where("PD.student_id", $student_id);
        $this->db->where("PD.sbi_payment_status", 1);
        $query = $this->db->get();
        $result = $query->result();
        return $result;
