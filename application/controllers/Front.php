 ...
 public function get_course_list()
   ...
   public function generate_student_id()
    ...
    public function addpayment() {
        $user_id = $this->session->userdata("userid"); 
        $this->session->set_userdata('payment_type', 'hostel_fees');//28112020
        if ($this->input->post()) {
    ...
    public function addstudentprofile() {
        ...
        if($flg == 'error') {
            ...
            $this->session->set_userdata('student_auto_id', $flg);//28112020
    ...
    //28112020
    public function addpaymentApplicationFee() {
        // $user_id = $this->session->userdata("userid");
        $this->session->set_userdata('payment_type', 'application_fee');

        if ($this->input->post()) {
            $amount = $this->input->post('application_amount');
            $student_id =  $this->input->post('student_id');
            $response = $this->algorithm($amount,$student_id);
            $ref_id = $response['ref_id'];
            $save = array(
                'student_id' => $this->input->post('student_id'),
                'total_amount' => $this->input->post('application_amount'),
                'total_paid' => $this->input->post('application_amount'),
                'total_overdue' => '0.00',
                'form_type' => 'application_fee',
                'ref_id' =>$ref_id
            );
            $this->db->insert(tablename('payment_details'), $save);
            $flg = $this->db->insert_id();
            ...
            if($flg){
                ...
                redirect('student-submitted-form');//check
            }
    ...
    //30112020
    // copy from studentpaymentWithOutAppFee
    public function studentpaymentWithAppFee($user_id,$payment_id) {
        $data = array();
        $data['user_id']  = $user_id;
        $data['payment_id']  = $payment_id;//change
        $data['student_details']  = $this->Frontmodel->load_all_student_data($user_id);
        $state_id = $data['student_details']->state_id;
        $state = $this->db->select('*')->where('id',$state_id)->get('state')->row();
        ...
        $this->data = array();
        $this->data['user_id']  = $user_id;
        $this->data['payment_id']  = $payment_id; //change
        $this->data['student_details']  = $this->Frontmodel->load_all_student_data($user_id);
  ...
  return ucwords($result) . "" . ucwords($points)." Only";
    ...
    public function thankyou(){
        if(isset($_POST)){
            ...
            // $user_id = $this->session->userdata("userid");//28112020
            $user_id = $this->session->userdata("userid") ? $this->session->userdata("userid") : $this->session->userdata("student_auto_id");//28112020
            $payment_type = $this->session->userdata("payment_type");//28112020
            log_message('info', "state: 3rd state thankyou Prod Transactions, student id: $user_id, decrypt: $decrypt , desc: just redirect return url , payment_type: $payment_type");
            ...
            if($status == 'Success'){
               ...
               $checkSum_double = $hash_map_double['checksum'];
                ...
                if($status_double == 'Success'){
                    ...
                    $resultWhatPayment = $this->Frontmodel->getWhatPayment($ref_id); 
                    //28112020
                    if($resultWhatPayment->form_type == 'application_fee'){
                        // $this->Frontmodel->update_student_status($user_id);
                        // $this->session->unset_userdata('student_auto_id');
                        // $this->session->unset_userdata('payment_type');
                        // redirect('student-form-done-fee/'.$user_id.'/'.$transaction_id);//30112020
                        redirect('student-form-done-fee/'.$resultWhatPayment->student_id.'/'.$transaction_id);//15012021
                    }
