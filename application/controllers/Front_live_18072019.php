...
class Front extends MY_Controller {
    ...
    public function index() {
        $this->data                     = array();
        $this->data['notice_list']      = $this->Frontmodel->get_result_data('notice', ['is_active' => '1']);
 ...
 public function get_course_list()
   ...
   public function generate_student_id()
    ...
    public function noticedetails() {
        $notice_details                 = $this->Frontmodel->get_result_data('notice', ['is_active' => '1']);
...
//echo "<pre>"; print_r($department); die;
  ...
  return ucwords($result) . "" . ucwords($points)." Only";
    ...
    {
        $student_id= $this->input->post('student_id'); 
        $student = $this->Frontmodel->get_row_data('student', ['student_id' => $student_id]);       
        if(!empty($student))
        {
            echo 'true'; exit();
        }
    ...
    //     $ch     = curl_init();  
    //     curl_setopt($ch,CURLOPT_URL,'http://103.16.101.52:8080/sendsms/bulksms?username=swbs-viswa&password=viswa123&type=0&dlr=1&destination=919733611035&source=SKETCH&message='.urlencode($message));
    ...
    public function thankyou(){
        if(isset($_POST)){
            ...
            $transaction_id = explode("=",$decArr[0]);
            $amount = explode("=",$decArr[1]);
            $ref_id = explode("=",$decArr[2]);
            $status = explode("=",$decArr[3]);
            $desc = explode("=",$decArr[4]);
            $transactiontime_date = explode("=",$decArr[5]);
            $checkSum = explode("=",$decArr[6]);
            ...
            if($status[1] == 'Success'){
                ...
                $requiredData = "ref_id=$ref_id[1]|amount=$amount[1]";
                $checkSum = md5($requiredData);
                ...
                if ($result === FALSE) {
                    $result = $this->Frontmodel->delete_temp_payment($user_id,$ref_id[1]); 
                    if($result){
                ...
                $transaction_id_double = explode("=",$decArrDouble[0]);
                $ref_id_double = explode("=",$decArrDouble[1]);
                $amount_double = explode("=",$decArrDouble[2]);
                $status_double = explode("=",$decArrDouble[3]);
                $desc_double = explode("=",$decArrDouble[4]);
                $checkSum_double = explode("=",$decArrDouble[5]);
                ...
                if($status_double[1] == 'Success'){
                        $result = $this->Frontmodel->update_payment_status($user_id,$ref_id[1],$transaction_id[1],$result); 
                    if($result){
                    ...
                    redirect('student-payment-details/'.$ref_id[1]);
                }else{
                     $result = $this->Frontmodel->delete_temp_payment($user_id,$ref_id[1]); 
                    if($result){
    ...
    public function print_hos_payment_invoice($paymentId){
        $data['invoice_details']  = $this->Frontmodel->hos_payment_invoice_details($paymentId);
        // print_r($data);die();
