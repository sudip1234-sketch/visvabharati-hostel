...
class Hostelmodel extends CI_Model {
    ...
    public function getHostel($hostel_id){
        $this->db->select('st.*');
        $this->db->from(tablename('hostel_rooms_seats')." as st");
        $this->db->where('st.hostel_id', $hostel_id);
        $this->db->where('st.is_active', 1);
        $query = $this->db->get();
        return $query->result();
    }

    public function getHostelDetailsData($hostel_id,$block_id,$floor_id,$room_no,$beds_nos){
        $this->db->select('st.student_id,st.full_name,st.hostel_code,st.block_no,st.floor_no,st.room_no,st.bed_no,dept.department_name,course.course_name,inst.institute_name,H.hostel_name,st.hostel_id,st.semester');
        $this->db->from(tablename('student')." as st");
        $this->db->join(tablename('institute')." as inst","inst.id = st.institute_id");
        $this->db->join(tablename('department')." as dept","dept.id = st.department_id");
        $this->db->join(tablename('course')." as course","course.id = st.course_id");
        $this->db->join(tablename('hostel')." as H","H.id = st.hostel_name");
        $this->db->where('st.hostel_name', $hostel_id);
        $this->db->where('st.block_no', $block_id);
        $this->db->where('st.floor_no', $floor_id);
        $this->db->where('st.room_no', $room_no);
        $this->db->where('st.bed_no', trim($beds_nos));
        $this->db->where('st.is_approved', 1);
        // $this->db->where('st.is_cancelled', 0);
