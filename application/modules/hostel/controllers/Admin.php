...
class Admin extends MX_Controller {
    ...
    public function download_hostel_report($hostel_id){
        $hostelData = $this->Hostelmodel->getHostel($hostel_id);
        $hostelArr = [];
        $i = 0;
        foreach ($hostelData as $key => $hostel) {
            $roomArr = explode(',', $hostel->beds_nos);
            foreach ($roomArr as $key => $value) {
                $block_id = $hostel->block_id;
                $floor_id = $hostel->floor_id;
                $room_no = $hostel->room_no;
                $hostelReport = $this->Hostelmodel->getHostelDetailsData($hostel_id,$block_id,$floor_id,$room_no,$value);
               $hostelArr[$i]['student_id'] = @$hostelReport->student_id;
               $hostelArr[$i]['full_name'] = @$hostelReport->full_name;
               $hostelArr[$i]['institute_name'] = @$hostelReport->institute_name;
               $hostelArr[$i]['department_name'] = @$hostelReport->department_name;
               $hostelArr[$i]['course_name'] = @$hostelReport->course_name;
               $hostelArr[$i]['hostel_id'] = @$hostelReport->hostel_id;
               $hostelArr[$i]['semester'] = @$hostelReport->semester;
               $hostelArr[$i]['hostel_name'] = @$hostelReport->hostel_name;
               $hostelArr[$i]['hostel_code'] = @$hostelReport->hostel_code;
               $hostelArr[$i]['block_no'] = $block_id;
               $hostelArr[$i]['floor_no'] = $floor_id;
               $hostelArr[$i]['room_no'] = $room_no;
               $hostelArr[$i]['bed_no'] = $value;
               $i++;
               
            }
        }

         $this->data['all_data'] = $hostelArr;
