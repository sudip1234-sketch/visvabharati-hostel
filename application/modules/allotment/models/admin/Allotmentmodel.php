...
class Allotmentmodel extends CI_Model {
    ...
    //11122020
    public function allotment_cancel($id, $cancel_reason) {
        $this->db->select('st.bed_no, hrs.booked_bed_nos, hrs.id');
        $this->db->from(tablename('student'). " as st");
        $this->db->join(tablename('hostel_rooms_seats')." as hrs","hrs.hostel_id = st.hostel_id AND hrs.block_id = st.block_no AND hrs.floor_id = st.floor_no AND hrs.room_no = st.room_no");
        $this->db->where('st.id', $id);
        $query = $this->db->get();
        $result = $query->row();

        $booked_bed_arr = explode(",",$result->booked_bed_nos);
        foreach (array_keys($booked_bed_arr, $result->bed_no) as $key) {
            unset($booked_bed_arr[$key]);
        }

        $booked_bed_string = implode(',', $booked_bed_arr);

        $this->db->update(tablename('hostel_rooms_seats'), array('booked_bed_nos' => $booked_bed_string), ['id' => $result->id]);
 ...
 <tr>
   <td style="text-align: center; vertical-align: middle; padding: 0px 25px 25px 25px; background: #1a1819; color: #fff; font-family: sans-serif; font-size: 20px;">
    ...
    public function getDepartment(){
        ...
        $this->db->from(tablename('department'));
        $this->db->where('is_active','Y');
