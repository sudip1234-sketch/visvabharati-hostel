 ...
 */
class Reissuecardmodel extends CI_Model {
    ...
    public function load_all_data() {
        ...
        $this->db->select("rsc.*,stu.full_name,stu.student_id as studentid"); 
        $this->db->from(tablename('reissue_student_card') . ' as rsc');     
        $this->db->join("student as stu","stu.id = rsc.student_id",'LEFT'); 
 ...
 public function load_all_payment_mess() {
    $current_month ="(MONTH(`pd`.`plan_month_end`) = MONTH(CURRENT_DATE()) AND YEAR(`pd`.`plan_month_end`) = YEAR(CURRENT_DATE()))";
     ...
     public function reissue_card() {
      ...
      $this->db->select('*');
      $this->db->from(tablename('reissue_student_card'));
      $this->db->where('id', $id);
      ...
      if (!empty($result)) {
          ...
          $update = array('is_issued' => $issued, 'is_new' => 0);
          $this->db->where('id', $id);

          if ($this->db->update(tablename('reissue_student_card'), $update)) {
              return 1;
          } else {
              return;
          }
