...
class Complaintmodel extends CI_Model {
    ...
    public function load_all_data() {
        $this->db->select('ad.*');
        $this->db->from(tablename('complaint_type')." as ad");
    ...
    public function status() {
      ...
      if (!empty($result)) {
          ...
          if ($this->db->update(tablename('complaint_type'), $update)) {
              return 1;
          } else {
              return;
          }
     ...
     */
     ...
     public function modify($id = '') {
        ...
        if (!empty($id)) {
            ...
            $this->db->where('id', $id)->update(tablename('complaint_type'), $data);
            return $id;
        } else {
            $data = array(
                'complaint_name'              => $complaint_name
            ...
            $this->db->insert(tablename('complaint_type'), $data);
