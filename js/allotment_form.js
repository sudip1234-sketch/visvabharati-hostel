...
//12112020
jQuery.validator.addMethod("pincodenoteapply", function(pincode, element) {
    console.log('pincode: '+ pincode);
    if( pincode == '731204' || pincode == '731235') {
        return false;
    }else{
        return true;
    }
}, "<br /> Currently this pin is not applied.");
...
function calculate_final_score(distance_score, last_final_exam_score) {
    console.log('distance_score=>'+ distance_score);
    console.log('last_final_exam_score=>'+ last_final_exam_score);
...
$("document").ready(function(){
    ...
    $("#country_div").hide();
    $('#passport_div').hide();
    $("#passport_valid_div").hide();
    $('#visa_div').hide();
    $("#date_of_issue_of_VISA_div").hide();
    $('#date_of_visa_valid_till_div').hide();
    ...
    $( "#date_of_passport_valid" ).datepicker({
      changeMonth: true,
      changeYear: true,
    });

    $( "#date_of_issue_of_VISA" ).datepicker({
      changeMonth: true,
      changeYear: true,
      yearRange: "-50:+0",
    });

    $( "#date_of_visa_valid_till" ).datepicker({
      changeMonth: true,
      changeYear: true,
    });

    $("#addformstudent").validate({
        ignore: ".ignore",
        rules:{
            student_id: {
                "required":true,
                "studentid":true,
                remote: {
                    url:"/vb-live/check-student-id-front",
                    type: "post",
                    data: {
                        field:"student_id",
                        value: function() {
            ...
            country_id: {//09112020
                "required":"#nationality_type_foreign:selected"
            },
            passport_no: {//09112020
                "required":"#nationality_type_foreign:selected"
            },
            date_of_passport_valid: {//09112020
                "required":"#nationality_type_foreign:selected"
            },
            visa_id: {//09112020
                "required":"#nationality_type_foreign:selected"
            },
            date_of_issue_of_VISA: {//09112020
                "required":"#nationality_type_foreign:selected"
            },
            date_of_visa_valid_till: {//09112020
                "required":"#nationality_type_foreign:selected"
            }
        },
        messages:{
            ...
            date_of_passport_valid:{ //09112020
                "required":"Please select Passport valid till."
            },
            visa_id:{ //09112020
                "required":"Please enter VISA Number."
            },
            date_of_issue_of_VISA:{ //09112020
                "required":"Please select date of issue of VISA."
            },
            date_of_visa_valid_till:{ //09112020
                "required":"Please select date of VISA valid till."
            }
        }
    });
    ...
    $("#check_student_exist").click(function() {
        ...
        $.ajax({ 
            type: 'POST',
            url: "/vb-live/check-student-exist",
    ...
    $('#nationality_type').change(function() {
        if(this.value == 'indian') {
            ...
            $("#country_div").show();
            $('#passport_div').show();
            $("#passport_valid_div").show();
            $('#visa_div').show();
            $("#date_of_issue_of_VISA_div").show();
            $('#date_of_visa_valid_till_div').show();
        }
    })
});